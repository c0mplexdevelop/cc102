...
public class Game extends Application {
    ...
    private List<Rectangle> createFXBoard(List<Rectangle> rects, Scene scene, Pane root) {
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 3; col++) {
                // Rect takes the first 2 arguments as WIDTH AND LENGTH, with the last one as
                // Color.
                ...
                // Get the respective coordiantes of the rectangles, then divide it by its
                // length to get the row index
                int boardRow = (int) (rect.getY() / rect.getWidth()); // We get Y as rows go downward, and X doesnt
                                                                      // change
                ...
                rect.setOnMouseClicked(mouseEvent -> {
                    ...
                    if (board.checkIfValid(boardRow, boardCol)) {
                        ...
                        /*
                         * To center the text on the rectangle, we get the rect's X coordinate and the
                         * third of its length
                         * Then, we add it to the LENGTH OF THE TEXT within the pixel (pixel length,
                         * like rect.getWidth())
                         * Now its centered in the x axis, do the same for col but instead of addition,
                         * s
                         */
                        double centerX = (rect.getX() + rect.getWidth() / 3)
                                + (boardMove.getLayoutBounds().getWidth() / 2);
                        double centerY = (rect.getY() + rect.getHeight() / 2)
                                - (boardMove.getLayoutBounds().getHeight() / 2);
    ...
    public void restart(Pane root, Scene primaryScene, Text gameOverMessage) {
        primaryScene.setFill(backgroundColor);
        // Update the currentTurnText properly, as if X won, it remains as O after
        // restart
        setCurrentTurn(currentTurnText, currentTurn);
    ...
    public void drawLineOnRowWon(Board board, Pane root, Scene scene, int winningRow) {
        // Get the index position of the correct Rect, we go down as checking for rows
        // is down
        // This multiplies 3 by 0, 1, or 2. If its 0, then we check 0,1,2; if its 1, we
        // check 3,4,5; and if its 2, we check 6,7,8

        double beginningPosition = scene.getWidth() / 3 * winningRow + 100;
        double rectHalfWidth = 100;

        Line winningLine = new Line(0, beginningPosition + rectHalfWidth - 2, scene.getWidth(),
                beginningPosition + rectHalfWidth);
    ...
    public void restartIfLetterWon(Pane root, Scene primaryScene, Text gameOverMessage) {
        ...
        PauseTransition transition = new PauseTransition(Duration.seconds(0.84));
        transition.setOnFinished(event -> {
            // We clear the group since it would infinitely expand and take alot of memory.
            // Clearing it, then rebuilding it is easier
            root.getChildren().clear();
            rects = createFXBoard(rects, primaryScene, root);
            board.board = createBoard();
            currentTurn = "X";
    ...
    public void drawLineOnColumnWon(Board board2, Pane root, Scene primaryScene, int winningCol) {
        ...
        Line winningLine = new Line(beginningPosition + rectHalfWidth - 3, 105, beginningPosition + rectHalfWidth + 3,
                primaryScene.getHeight());
        // Its 105 since we put it within the rectangle, its probably like (Stroke Width
        // / # of suqares) + rect stroke width + 1
        // For the above, we added +3 to centralize the line stroke on the center.
        // Essentially, just do strokeWidth/3
        winningLine.setStroke(winningLineColor);
        winningLine.setStrokeWidth(10);
        winningLine.toBack();
    ...
    public void stopRectanglesFromReceivingClicks(List<Rectangle> rects) {
        // After winning, the game, we remove the functionality to be clicked from the
        // rectangles
